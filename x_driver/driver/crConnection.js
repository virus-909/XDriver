...
class CRSession extends import_events.EventEmitter {
  ...
  async __re__emitExecutionContext({
    ...
    const fixMode = process.env["XDRIVER_PATCHES_RUNTIME_FIX_MODE"] || "addBinding";
    const utilityWorldName = process.env["XDRIVER_PATCHES_UTILITY_WORLD_NAME"] !== "0" ? process.env["XDRIVER_PATCHES_UTILITY_WORLD_NAME"] || "util" : "__playwright_utility_world__";
    process.env["XDRIVER_PATCHES_DEBUG"] && console.log(`[xdriver-patches][crSession] targetId = ${targetId}, world = ${world}, frame = ${frame ? "Y" : "N"}, fixMode = ${fixMode}`);
    let getWorldPromise;
  ...
  async __re__getMainWorld({ client, frameId, isWorker = false }) {
    ...
    process.env["XDRIVER_PATCHES_DEBUG"] && console.log(`[xdriver-patches][getMainWorld] binding name = ${randomName}`);
    await client.send("Runtime.addBinding", {
      name: randomName
    });
    const bindingCalledHandler = ({ name, payload, executionContextId }) => {
      process.env["XDRIVER_PATCHES_DEBUG"] && console.log("[xdriver-patches][bindingCalledHandler]", {
        name,
        payload,
        executionContextId
      });
    ...
    process.env["XDRIVER_PATCHES_DEBUG"] && console.log(`[xdriver-patches][getMainWorld] result:`, { contextId });
    return contextId;
  }
  async __re__getIsolatedWorld({ client, frameId, worldName }) {
    ...
    process.env["XDRIVER_PATCHES_DEBUG"] && console.log(`[xdriver-patches][getIsolatedWorld] result:`, createIsolatedWorldResult);
